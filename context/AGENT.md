...
- **Review System & JWT Role Inference:**
  - *Issue:* Ensuring users can only rate their own completed/cancelled exchanges without easily spoofing roles (e.g. buyer posing as seller).
  - *Solution:* Use `Principal.getName()` in Spring Boot to fetch the real authenticated user. The backend compares `uid` against `idVendedor` and `idComprador` of the `Intercambio` to automatically assign the rating to the correct column (`calificacionAlComprador` vs `calificacionAlVendedor`). Never trust a "role" string passed from the Android client. This ensures tamper-proof reviews. Android handles this by dynamically showing a `BottomSheetDialogFragment` at the end of the lifecycle.
